# compact-core Implementation Knowledge

This folder explains how the general concepts are applied inside
`compact-core`.

- [Architecture](01-architecture.md)
- [Build roadmap](02-build-roadmap.md)
- [v0.2 streaming index](03-v0.2-streaming-index.md)
- [Rules and reminders](04-rules-and-reminders.md)
- [v0.2 streaming detail](streaming/README.md)
- [v0.3 advanced column compression](v0.3/README.md)

Keep repo-specific decisions here. Keep reusable concepts in
[`../concepts`](../concepts/README.md).
